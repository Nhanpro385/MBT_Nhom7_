<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="public/css/style.css">
    <title>Document</title>
</head>

<body>
    <section class="">
        <div class="container box_content">
            <div class="side_bar">
                <h1 class="heading_sidebar_filter">
                    bộ lọc

                </h1>
                <div class="filter_list">
                    <div class="custom-select">
                        <div class="custom-select-label">
                            <span>ngôn ngữ</span>
                            <i class="fa-solid fa-plus"></i>
                        </div>
                        <ul class="list_sidebar_filter">
                            <li class="sidebar_filter_item active_fileter">việt nam</li>
                            <li class="sidebar_filter_item">anh</li>
                            <li class="sidebar_filter_item">nhật</li>
                            <li class="sidebar_filter_item">trung quốc</li>
                            <li class="sidebar_filter_item">hàn quốc</li>
                        </ul>
                    </div>
                    <div class="custom-select">
                        <div class="custom-select-label">
                            <span>thể loại</span>
                            <i class="fa-solid fa-plus"></i>
                        </div>
                        <ul class="list_sidebar_filter">
                            <li class="sidebar_filter_item active_fileter">hành động</li>
                            <li class="sidebar_filter_item">tình cảm</li>
                            <li class="sidebar_filter_item">hài hước</li>
                            <li class="sidebar_filter_item">kinh dị</li>
                            <li class="sidebar_filter_item">tâm lý</li>
                        </ul>
                    </div>
                    <div class="custom-select">
                        <span>giá tiền</span>
                        <i class="fa-solid fa-plus"></i>

                    </div>
                </div>
            </div>
            <div class="list_content">
                <ul class="list_tiket row ">
                    <li class="ticket_item col-3">
                        <img src="https://d1j8r0kxyu9tj8.cloudfront.net/files/1594785807J145RGZocrbiYIz.jpg" alt="" class="ticket_img">
                        <div class="ticket_title">spider man</div>
                        <div class="btn_ticket">
                            <a href="" class="btn_ticket_detail">chi tiết</a>
                            <a href="" class="btn_ticket_buy">mua vé</a>
                        </div>
                    </li>
                    <li class="ticket_item col-3">
                        <img src="https://d1j8r0kxyu9tj8.cloudfront.net/files/1594785807J145RGZocrbiYIz.jpg" alt="" class="ticket_img">
                        <div class="ticket_title">spider man</div>
                        <div class="btn_ticket">
                            <a href="" class="btn_ticket_detail">chi tiết</a>
                            <a href="" class="btn_ticket_buy">mua vé</a>
                        </div>
                    </li>
                    <li class="ticket_item col-3">
                        <img src="https://d1j8r0kxyu9tj8.cloudfront.net/files/1594785807J145RGZocrbiYIz.jpg" alt="" class="ticket_img">
                        <div class="ticket_title">spider man</div>
                        <div class="btn_ticket">
                            <a href="" class="btn_ticket_detail">chi tiết</a>
                            <a href="" class="btn_ticket_buy">mua vé</a>
                        </div>
                    </li>
                    <li class="ticket_item col-3">
                        <img src="https://d1j8r0kxyu9tj8.cloudfront.net/files/1594785807J145RGZocrbiYIz.jpg" alt="" class="ticket_img">
                        <div class="ticket_title">spider man</div>
                        <div class="btn_ticket">
                            <a href="" class="btn_ticket_detail">chi tiết</a>
                            <a href="" class="btn_ticket_buy">mua vé</a>
                        </div>
                    </li>
                    <li class="ticket_item col-3">
                        <img src="https://d1j8r0kxyu9tj8.cloudfront.net/files/1594785807J145RGZocrbiYIz.jpg" alt="" class="ticket_img">
                        <div class="ticket_title">spider man</div>
                        <div class="btn_ticket">
                            <a href="" class="btn_ticket_detail">chi tiết</a>
                            <a href="" class="btn_ticket_buy">mua vé</a>
                        </div>
                    </li>
                    <li class="ticket_item col-3">
                        <img src="https://d1j8r0kxyu9tj8.cloudfront.net/files/1594785807J145RGZocrbiYIz.jpg" alt="" class="ticket_img">
                        <div class="ticket_title">spider man</div>
                        <div class="btn_ticket">
                            <a href="" class="btn_ticket_detail">chi tiết</a>
                            <a href="" class="btn_ticket_buy">mua vé</a>
                        </div>
                    </li>
                    <li class="ticket_item col-3">
                        <img src="https://d1j8r0kxyu9tj8.cloudfront.net/files/1594785807J145RGZocrbiYIz.jpg" alt="" class="ticket_img">
                        <div class="ticket_title">spider man</div>
                        <div class="btn_ticket">
                            <a href="" class="btn_ticket_detail">chi tiết</a>
                            <a href="" class="btn_ticket_buy">mua vé</a>
                        </div>
                    </li>
                    <li class="ticket_item col-3">
                        <img src="https://d1j8r0kxyu9tj8.cloudfront.net/files/1594785807J145RGZocrbiYIz.jpg" alt="" class="ticket_img">
                        <div class="ticket_title">spider man</div>
                        <div class="btn_ticket">
                            <a href="" class="btn_ticket_detail">chi tiết</a>
                            <a href="" class="btn_ticket_buy">mua vé</a>
                        </div>
                    </li>
                </ul>
                <nav aria-label="Page navigation example ">
                    <ul class="pagination justify-content-center">
                        <li class="page-item"><a class="page-link" href="#">lùi trang</a></li>
                        <li class="page-item"><a class="page-link" href="#">1</a></li>
                        <li class="page-item"><a class="page-link" href="#">2</a></li>
                        <li class="page-item"><a class="page-link" href="#">3</a></li>
                        <li class="page-item"><a class="page-link" href="#">trang kế</a></li>
                    </ul>
                </nav>
            </div>
        </div>
    </section>
</body>
<script>
    jQuery.fn.extend({
        setMenu: function() {
            return this.each(function() {
                var containermenu = $(this);

                var itemmenu = containermenu.find('.custom-select-label');
                itemmenu.click(function() {
                    var submenuitem = containermenu.find('.list_sidebar_filter');
                    submenuitem.slideToggle(500);

                });

                $(document).click(function(e) {
                    if (!containermenu.is(e.target) &&
                        containermenu.has(e.target).length === 0) {
                        var isopened =
                            containermenu.find('.list_sidebar_filter').css("display");

                        if (isopened == 'block') {
                            containermenu.find('.list_sidebar_filter').slideToggle(500);
                        }
                    }
                });



            });
        },

    });

    $('.custom-select').setMenu(); //
</script>

</html>